...
const SELECT_boardlimit =
    "SELECT @rownum:=@rownum+1 as number, board_title, board_box,board_date, board_filename, board_views, user.user_id, user.name, user.gender FROM board INNER JOIN user ON board.user_id = user.user_id ORDER BY board_date DESC LIMIT ?,?";
