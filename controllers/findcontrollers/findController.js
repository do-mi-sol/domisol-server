...
module.exports = {
    idFind: async (req, res, next) => {
        ...
        try {
            ...
            if (search_data[0].length === 0) {
                return errorMsg(res, 300, "아이디 찾기 실패. 입력하신 정보와 일치하는 아이디가 존재하지 않습니다.");
            }
    ...
    passwordFind: async (req, res, next) => {
        user = req.body;
        try {
            const [search_data] = await pool.query(SQL.SELECT_name_email_userid, [user.name, user.email, user.user_id]);
